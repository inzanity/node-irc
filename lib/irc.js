...
Client.prototype.disconnect = function(message, callback) {
    ...
    self.conn.end();
    if (self._floodTimeoutId) {
        clearTimeout(self._floodTimeoutId);
        self._floodTimeoutId = undefined;
    }
    if (self._floodClearTimeoutId) {
        clearTimeout(self._floodClearTimeoutId);
        self._floodClearTimeoutId = undefined;
    }
};
...
Client.prototype.activateFloodProtection = function(interval) {
    ...
    self.floodProtectionImmediateCount = 0;
    self.opt.floodProtection = true;
    ...
    this.send = function() {
        if (self._floodClearTimeoutId)
            clearTimeout(self._floodClearTimeoutId);
        if (!self._floodTimeoutId) {
            if (self.floodProtectionImmediateCount++ < self.opt.floodProtectionImmediate || new Date() - prevSentAt > safeInterval) {
                self._floodClearTimeoutId = setTimeout(clearFlood, Math.max(self.opt.floodProtectionImmediate, self.floodProtectionImmediateCount) * self.opt.floodProtectionDelay);
                origSend.apply(self, arguments);
                prevSentAt = new Date();
                return;
            }

            self._floodTimeoutId = setTimeout(dequeue, new Date() - prevSentAt + safeInterval);
        }
    ...
    dequeue = function() {
        self._floodTimeoutId = undefined;
        origSend.apply(self, cmdQueue.shift());
        prevSentAt = new Date();
        if (cmdQueue.length) 
            self._floodTimeoutId = setTimeout(dequeue, safeInterval);
        else
            self._floodClearTimeoutId = setTimeout(clearFlood, safeInterval * self.opt.floodProtectImmediate);
    };
};
...
Client.prototype._splitLongLines = function(words, maxLength, destination, ellipsis) {
    if (words.length == 0) {
        return destination;
    }
    ...
    ellipsis = ellipsis || '';
    var re = new RegExp('^(.{1,' + (maxLength - ellipsis.length) + '})(?:\\s+)');
    var cutPos = 0;
    while (cutPos < words.length - maxLength) {
        console.log(words.substring(cutPos));
        var m;
        if ((m = words.substring(cutPos).match(re))) {
            destination.push(m[1] + ellipsis);
            cutPos += m[0].length;
        } else {
            destination.push(words.substr(cutPos, maxLength - ellipsis.length) + ellipsis);
            cutPos += maxLength - ellipsis.length;
        }
    }
    destination.push(words.substr(cutPos));
    return destination;
};
...
Client.prototype._splitMessage = function(target, text) {
    ...
    if (text) {
        return text.toString().split(/\r?\n/).filter(function(line) {
            ...
            return self._splitLongLines(line, maxLength, [], self.opt.splitEllipsis);
        }).reduce(function(a, b) {
            return a.concat(b);
        }, []);
    }
